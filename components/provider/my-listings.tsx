...
export function MyListings({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    setHousing(getHousingList().filter((h) => h.createdBy === "Siz"))
    setJobs(getJobsList().filter((j) => j.createdBy === "Siz"))
    setMaterials(getMaterialsList().filter((m) => m.createdBy === "Siz"))
  }, [refreshKey])

  const handleRemoveHousing = (id: string) => {
    removeHousing(id)
